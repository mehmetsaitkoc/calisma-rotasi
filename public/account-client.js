...
async function action(name){
 ...
 if(name==='delete-prompt')adapter.modal('Hesabını kalıcı olarak sil',`<form id="account-delete"><p class="dialog-desc">Profilin, çalışma geçmişin ve sonuçların sunucudan silinir. Bu işlem geri alınamaz. Önce hesabının yedeğini indirebilirsin.</p><label class="field">Onaylamak için parolan<input type="password" name="password" autocomplete="current-password" required></label><label class="field mt">SİL yazarak onayla<input name="confirm" pattern="SİL" required></label><p class="form-error" role="alert" id="account-error"></p><div class="button-row"><button type="button" class="btn ghost" data-account-action="export">Önce yedeği indir</button><button type="submit" class="btn danger">Hesabımı kalıcı olarak sil</button></div></form>`);
}
