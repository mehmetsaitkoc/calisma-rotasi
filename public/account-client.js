...
let adapter,user=null,csrf='',nativeToken='',cache=null,cacheStamp=null,epoch=0,busy=false,timer=0,status='guest',problem='',conflict=null,guest=null;
...
root.addEventListener('online',()=>{if(user)void flush();else if(adapter)void boot();});
...
root.RotaAccount=Object.freeze({request,json,changed,flush,ready,open:accountPanel,login:loginPanel,get user(){return user?{...user}:null;},get status(){return status;},bind(value){if(adapter)throw Error('Hesap bağlantısı zaten kurulu.');adapter=value;void boot();}});
})(window);
