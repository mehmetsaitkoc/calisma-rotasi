...
function validatePilotPayload(payload){
  ...
  if(!['kpss','yks'].includes(payload.exam)||!isDate(payload.startDate)||!isDate(payload.generatedDate))throw Error('Pilot sınav veya tarih bilgisi geçersiz.');
  if(payload.lastActiveDate&&!isDate(payload.lastActiveDate))throw Error('Son aktif tarih geçersiz.');
  if(!Array.isArray(payload.snapshots)||payload.snapshots.length>8)throw Error('Pilot snapshot listesi geçersiz.');
  const seen=new Set();
  for(const s of payload.snapshots){
    if(!s||!CHECKPOINTS.includes(s.checkpoint)||seen.has(s.checkpoint))throw Error('Pilot checkpoint geçersiz veya yineleniyor.');seen.add(s.checkpoint);
    if(!isDate(s.targetDate)||!isDate(s.capturedDate))throw Error('Pilot snapshot tarihi geçersiz.');
...
function studentOverview(payload,asOfDate=payload.generatedDate){
  validatePilotPayload(payload);const latest=latestSnapshot(payload),alarms=pilotAlarms(payload,asOfDate),milestones=CHECKPOINTS.map(cp=>milestoneStatus(payload,cp,asOfDate));
  return {
