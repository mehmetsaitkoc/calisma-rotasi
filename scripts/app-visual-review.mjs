...
async function assertReferenceHero(page,label,{mobile=false}={}){
  ...
  if(mobile){
    ...
    assert.ok(geometry.beforeBackground.includes('hero-journey-final.webp'),label+': mobile hero must use the photographic journey artwork');
    ...
    assert.ok(geometry.beforeBackground.includes('hero-journey-final.webp'),label+': desktop hero must use the photographic journey artwork');
