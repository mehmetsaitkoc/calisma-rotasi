...
assert.ok(C&&B&&Q&&P&&P2&&P3&&P4&&S,'KPSS v2 content modules must load');
...
const professional=[...P.tests,...P2.tests,...P3.tests,...P4.tests];
...
assert.equal(audit.tests,16);
assert.equal(audit.questions,192);
assert.equal(audit.topics,4);
for(const topicId of ['k-tr-1','k-tr-2','k-tr-3','k-tr-4']){
...
console.log('KPSS content v2 passed: 3,672-question target + 192 approved topic questions across 4 Turkish topics + 30-question professional Turkish section pilot');
