...
const kpssBank=[...(globalThis.RotaKpssQuestionBank?.topicTests||[])];
const defs=[...legacySeeds,...kpssPractices,...kpssBank];
...
assert.equal(kpssBank.length,11,'Standardized KPSS bank must add one 12-question test for every Turkish topic in batch 1');
assert.equal(defs.length,59,'Original mini seeds + KPSS practices + standardized bank must compose without mutating legacy seeds');
...
for(const def of defs){
  ...
  else assert.ok([1,2,3].includes(n.setNo),'KPSS practice/bank sets may advance after legacy sets without collisions');
...
console.log('Mini catalog passed: legacy seeds + KPSS practices + standardized Turkish bank + original-only 4-set expansion scaffold');
