...
const burn=lfById['burnout-after-success'];assert.ok(burn.days.find(function(d){return d.appliedMode==='progress';}),'burnout persona never reached progress');assert.ok(burn.days.slice(25).some(function(d){return d.appliedMode!=='progress';}),'progress stuck after burnout');const burnEase=burn.days.find(function(d){return d.appliedMode==='ease';});if(!burnEase||burnEase.day>33)console.log('BURNOUT-DELAY-DIAG',JSON.stringify(burn.days.slice(19,41).map(function(d){return {day:d.day,phase:d.personaPhase,mode:d.appliedMode,state:d.studentState,completion:d.behaviorCompletion,recent:d.recentSubjectCompletion,previous:d.previousSubjectCompletion,collapse:d.completionCollapse,execution:d.execution,risk:d.risk};})));assert.ok(burnEase&&burnEase.day<=33,'burnout sustainable response was too late: '+(burnEase?.day||'never'));
{
