...
const burn=lfById['burnout-after-success'];assert.ok(burn.days.find(function(d){return d.appliedMode==='progress';}),'burnout persona never reached progress');assert.ok(burn.days.slice(25).some(function(d){return d.appliedMode!=='progress';}),'progress stuck after burnout');const burnEase=burn.days.find(function(d){return d.appliedMode==='ease';});assert.ok(burnEase&&burnEase.day<=33,'burnout sustainable response was too late: '+(burnEase?.day||'never'));
{
