...
try{
  ...
  assert.match(landingHtml,/intelligence-v1\.js/,'Landing page must load Intelligence V1');
  assert.match(landingHtml,/intelligence-bridge-v1\.js/,'Landing page must load the Intelligence runtime bridge');
  ...
  const intelligenceJs=await fetch(BASE+'/intelligence-v1.js');
  const intelligenceText=await intelligenceJs.text();
  assert.equal(intelligenceJs.status,200,'Intelligence V1 must be served');
  assert.match(intelligenceText,/RotaIntelligenceV1/);
  const intelligenceBridgeJs=await fetch(BASE+'/intelligence-bridge-v1.js');
  const intelligenceBridgeText=await intelligenceBridgeJs.text();
  assert.equal(intelligenceBridgeJs.status,200,'Intelligence bridge must be served');
  assert.match(intelligenceBridgeText,/RotaIntelligenceBridgeV1/);
