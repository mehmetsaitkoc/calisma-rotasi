...
function serve(req,res){
  ...
  fs.realpath(file,(err,real)=>{if(err)return json(res,404,{error:'Bulunamadı.'});const rel=path.relative(PUBLIC,real);if(rel==='..'||rel.startsWith('..'+path.sep)||path.isAbsolute(rel))return json(res,403,{error:'Yasak yol.'});fs.stat(real,(err,st)=>{if(err||!st.isFile())return json(res,404,{error:'Bulunamadı.'});res.writeHead(200,{'content-type':mime(real),'cache-control':'no-store'});const stream=fs.createReadStream(real);stream.on('error',()=>res.destroy());stream.pipe(res);});});
}
...
server.listen(PORT,HOST,()=>{
  ...
  console.log(runtimeApiKey ? `Rota Hoca AI açık · ${runtimeModel} · ${runtimeProfile}` : 'Rota Hoca AI yapılandırılmamış · sunucu ortam değişkenleriyle yapılandırılmalı');
});
