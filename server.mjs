...
const server=http.createServer(async (req,res)=>{
  ...
  // Safe CSP baseline for the inline-heavy legacy shell. A strict script/style CSP
  // needs a nonce/hash migration first; these directives harden framing, plugins,
  // base URL rewriting and form exfiltration without breaking the current app.
  res.setHeader('content-security-policy',"object-src 'none'; base-uri 'self'; form-action 'self'; frame-ancestors 'self'");
